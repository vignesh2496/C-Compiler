...
statement : expression ';'
          ...
          { 
			...
			printf("IF %s == TRUE THEN L%d ELSE L%d\nLABEL L%d:\n",$7,labels.top(),labels.top() + 1,labels.top() + 1);
			labels.pop();
		  }
		  | return_statement 
		  | io_statement
...
loop_statement : _FOR '(' expression ';' { labels.push(label); printf("LABEL L%d:\n",labels.top()); } expression ';'  expression ')' 
			   { 	
				    table.clear();
				    s.push(table);
				    
				    printf("IF %s == TRUE THEN L%d ELSE L%d\nLABEL L%d:\n",$6,labels.top() + 1,labels.top() + 2,labels.top()  + 1);
				    label += 3;
			   }
			   | _WHILE { labels.push(label); printf("LABEL L%d:\n",labels.top()); } '(' expression ')' 
			   { 	
				    table.clear();
				    s.push(table);
				    
				    printf("IF %s == TRUE THEN L%d ELSE L%d\nLABEL L%d:\n",$4,labels.top() + 1,labels.top() + 2,labels.top() + 1);
				    label += 3;
			   }
			   ;
...
if_statement : _IF '(' expression ')' 
			 { 	
				...
				labels.push(label); 
				printf("IF %s == TRUE THEN L%d ELSE L%d\nLABEL L%d:\n",$3,labels.top(),labels.top() + 1,labels.top());
				label += 3;
			 }
			 ;
...
void dispSymbolTable()
{
	...
	for(i = 0 ; i < v.size() ; i++)
	{
		...
		{
			(it->second).second = i;
			cout << "< " << it->first << " , " << (it->second).first << " , " << (it->second).second << " , " << (it->second).second * 1000 + address << " >" << endl;
			address += 4;
		}
	}
} 	
